...
LandmarkAtlasSegmentationFilter<TInputImage, TOutputImage>::GenerateData()
{
  ...
  //
  //        IndexType minIndThread{ static_cast<IndexValueType>(contentRegion.GetSize()[0]) +
  //        contentRegion.GetIndex()[0],
  //                                static_cast<IndexValueType>(contentRegion.GetSize()[1]) +
  //                                contentRegion.GetIndex()[1], static_cast<IndexValueType>(contentRegion.GetSize()[2])
  //                                + contentRegion.GetIndex()[2] };
  ...
  //  : resolutions = 2 default_affine =
  //    parameters.GetDefaultParameterMap("affine", resolutions)
  //      parameters.AddParameterMap(default_affine)
  //
  //      if (regType.value == 'bspline')
  //  : resolutions = 1 default_bspline =
  //      parameters.GetDefaultParameterMap("bspline", resolutions)
  ...
  elastixRM->SetLogToConsole(true);

  elastixRM->Update();

  std::cout << elastixRM->GetTransformParameterObject() << std::endl;

  using TransformixFilterType = itk::TransformixFilter<InputImageType>;
  TransformixFilterType::Pointer transformix = TransformixFilterType::New();
  transformix->SetMovingImage(atlasBone1);
  transformix->SetTransformParameterObject(elastixRM->GetTransformParameterObject());
